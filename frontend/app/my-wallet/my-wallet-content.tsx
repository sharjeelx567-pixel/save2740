...
import { CheckCircle2, ArrowUp, Download } from "lucide-react"
...
export function MyWalletPageContent() {
    ...
    return (
        <div className="flex h-screen bg-[#F8FAFC] overflow-hidden">
            ...
            <div key="desktop-sidebar" className="hidden lg:block h-full">
                <Sidebar />
            </div>

            <Sheet key="mobile-sidebar" open={isSidebarOpen} onOpenChange={setIsSidebarOpen}>
                <SheetContent side="left" className="p-0 w-64">
                    <Sidebar onClose={() => setIsSidebarOpen(false)} />
                </SheetContent>
            </Sheet>

            <TransactionNotificationContainer key="transaction-notifications" />

            <main key="main-content" className="flex-1 overflow-y-auto flex flex-col">
                            ...
                            {txLoading ? (
                                <Card className="bg-white border border-slate-100 rounded-2xl md:rounded-3xl p-6">
                                    <div className="space-y-4">
                                        {[...Array(3)].map((_, i) => (
                                            <div key={`skeleton-tx-${i}`} className="h-16 bg-slate-100 rounded-lg animate-pulse" />
                                        ))}
                                    </div>
                                </Card>
