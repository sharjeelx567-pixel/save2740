...
function GroupContributionPageContent() {
  ...
  const fetchGroups = async () => {
    ...
    try {
      ...
      // Transactions could be fetched separately or included
      // For now, empty transactions unless API supports it
      setTransactions([]);
  ...
  const handleJoinGroup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/groups/join', {
        ...
        body: JSON.stringify({ joinCode: joinData.referralCode })
      });
      ...
      // Update groups list
      const exists = groups.find(g => g.id === updatedGroup.id || g._id === updatedGroup._id);
      if (exists) {
        setGroups(groups.map(g => (g.id === updatedGroup.id || g._id === updatedGroup._id) ? updatedGroup : g));
      } else {
